...
const AddProjectModal = ({ setFetch, onClose }: Props) => {
    ...
    const handleCreateProject = async () => {
        createProject(project)
            .then(response => {
                router.push(`/project/${response.data}/board`);
                setFetch(true);
                onClose();
            })
            .catch(response => {
            ...
            <div className="max-w-[100vw] grid grid-rows-[1fr_15fr_1fr] h-full px-[3rem] py-[1rem]">
                ...
                <div>
                    <div className="mb-[.5rem] flex flex-col gap-2">
                        <Label className="text-[1.4rem] font-medium whitespace-nowrap overflow-hidden text-ellipsis"> Название проекта </Label>
                        <Input
                            onChange={(event) => {
                                setErrorMessage("");
                                setProject((prevState) => ({
                    ...
                    <div className="flex flex-col max-w-[calc(50vw - 6rem)]">
                        <Label className="text-black text-[1.4rem] font-medium whitespace-nowrap overflow-hidden text-ellipsis mt-[1.5rem] mb-[0.5rem]"> Описание проекта </Label>
                        <div
                ...
                <div>
                    <Button className="bg-sky-600 border border-sky-600 text-[1rem] text-white hover:bg-sky-700 ease-in-out" onClick={() => handleCreateProject()}> Создать проект </Button>
                </div>
            </div>
        </>
    );
